...
import com.cynosure.operit.util.LocaleUtils.LanguageCodes
...
class UserPreferencesManager private constructor(private val context: Context) {
    companion object {
        ...
        private val KEY_MENU_NAME_OVERRIDES = stringPreferencesKey("menu_name_overrides")
    ...
    suspend fun saveMenuNameOverride(key: String, name: String) {
        context.userPreferencesDataStore.edit { preferences ->
            val overrides = parseMenuNameOverrides(preferences[KEY_MENU_NAME_OVERRIDES])
            val updated = if (name.isBlank()) {
                overrides - key
            } else {
                overrides + (key to name)
            }
            preferences[KEY_MENU_NAME_OVERRIDES] = Json.encodeToString(updated)
        }
    }

    suspend fun resetMenuNameOverrides() {
        context.userPreferencesDataStore.edit { preferences ->
            preferences.remove(KEY_MENU_NAME_OVERRIDES)
        }
    }

    private fun parseMenuNameOverrides(json: String?): Map<String, String> {
    ...
    val menuNameOverrides: Flow<Map<String, String>> =
        context.userPreferencesDataStore.data.map { preferences ->
            parseMenuNameOverrides(preferences[KEY_MENU_NAME_OVERRIDES])
        }
