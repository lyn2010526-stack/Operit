...
import com.cynosure.operit.data.model.CharacterCardToolAccessConfig
import com.cynosure.operit.data.model.PromptTag
import com.cynosure.operit.data.model.TagType
import com.cynosure.operit.data.model.TavernCharacterCard
import com.cynosure.operit.data.model.TavernCharacterData
import com.cynosure.operit.R
import com.cynosure.operit.data.model.TavernExtensions
import com.cynosure.operit.data.model.OperitTavernExtension
import com.cynosure.operit.data.model.OperitAttachedTagPayload
import com.cynosure.operit.data.model.OperitCharacterCardPayload
...
class CharacterCardManager private constructor(private val context: Context) {
    ...
    companion object {
        ...
        fun serializeFilePaths(paths: List<String>): String {
            return paths.filter { it.isNotBlank() }.joinToString(FILE_PATH_SEPARATOR)
        }

        fun deserializeFilePaths(raw: String?): List<String> {
            return raw?.split(FILE_PATH_SEPARATOR)?.map { it.trim() }?.filter { it.isNotEmpty() } ?: emptyList()
        }
    ...
    private fun getCharacterCardFromPreferences(preferences: Preferences, id: String): CharacterCard {
        ...
        val customPromptFilePathsKey = stringPreferencesKey("character_card_${id}_custom_prompt_file_paths")
        ...
        return CharacterCard(
            ...
            customPromptFilePaths = deserializeFilePaths(preferences[customPromptFilePathsKey]),
    ...
    suspend fun createCharacterCard(card: CharacterCard): String {
        ...
        preferences[stringPreferencesKey("character_card_${id}_other_content_voice")] = newCard.otherContentVoice
            ...
            preferences[stringPreferencesKey("character_card_${id}_custom_prompt_file_paths")] = serializeFilePaths(newCard.customPromptFilePaths)
    ...
    suspend fun updateCharacterCard(card: CharacterCard) {
        ...
        preferences[stringPreferencesKey("character_card_${card.id}_other_content_voice")] = card.otherContentVoice
            ...
            preferences[stringPreferencesKey("character_card_${card.id}_custom_prompt_file_paths")] = serializeFilePaths(card.customPromptFilePaths)
    ...
    suspend fun combinePrompts(
        ...
        val combinedPrompt = buildString {
            ...
            characterCard.customPromptFilePaths.forEach { filePath ->
                if (filePath.isNotBlank()) {
                    val content = runCatching {
                        File(filePath).takeIf { it.exists() && it.canRead() }?.readText()?.trim()
                    }.getOrNull()
    ...
    private suspend fun upsertCharacterCardWithId(card: CharacterCard) {
        dataStore.edit { preferences ->
            ...
            preferences[stringPreferencesKey("character_card_${id}_custom_prompt_file_paths")] = serializeFilePaths(card.customPromptFilePaths)
