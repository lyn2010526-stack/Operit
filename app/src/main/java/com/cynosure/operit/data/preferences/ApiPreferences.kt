...
import com.cynosure.operit.data.model.ParameterCategory
import com.cynosure.operit.data.model.ParameterValueType
...
class ApiPreferences private constructor(private val context: Context) {
    ...
    companion object {
        ...
        // Key for Anti-Hallucination Guard
        val ENABLE_ANTI_HALLUCINATION = booleanPreferencesKey("enable_anti_hallucination")
        val ENABLE_FACT_GUARD = booleanPreferencesKey("enable_fact_guard")
        val ENABLE_OUTPUT_DISCIPLINE = booleanPreferencesKey("enable_output_discipline")
        val ENABLE_FILE_GUARD = booleanPreferencesKey("enable_file_guard")

        // Key for Self-Evolution Engine
        val ENABLE_SELF_EVOLUTION = booleanPreferencesKey("enable_self_evolution")
        ...
        // Default value for Anti-Hallucination Guard
        const val DEFAULT_ENABLE_ANTI_HALLUCINATION = false
        const val DEFAULT_ENABLE_FACT_GUARD = true
        const val DEFAULT_ENABLE_OUTPUT_DISCIPLINE = true
        const val DEFAULT_ENABLE_FILE_GUARD = true

        // Default value for Self-Evolution Engine
        const val DEFAULT_ENABLE_SELF_EVOLUTION = false
    ...
    // Flow for Anti-Hallucination Guard
    val enableAntiHallucinationFlow: Flow<Boolean> =
        context.apiDataStore.data.map { preferences ->
            preferences[ENABLE_ANTI_HALLUCINATION] ?: DEFAULT_ENABLE_ANTI_HALLUCINATION
        }

    val enableFactGuardFlow: Flow<Boolean> =
        context.apiDataStore.data.map { preferences ->
            preferences[ENABLE_FACT_GUARD] ?: DEFAULT_ENABLE_FACT_GUARD
        }

    val enableOutputDisciplineFlow: Flow<Boolean> =
        context.apiDataStore.data.map { preferences ->
            preferences[ENABLE_OUTPUT_DISCIPLINE] ?: DEFAULT_ENABLE_OUTPUT_DISCIPLINE
        }

    val enableFileGuardFlow: Flow<Boolean> =
        context.apiDataStore.data.map { preferences ->
            preferences[ENABLE_FILE_GUARD] ?: DEFAULT_ENABLE_FILE_GUARD
        }

    // Flow for Self-Evolution Engine
    val enableSelfEvolutionFlow: Flow<Boolean> =
        context.apiDataStore.data.map { preferences ->
            preferences[ENABLE_SELF_EVOLUTION] ?: DEFAULT_ENABLE_SELF_EVOLUTION
        }
    ...
    // Save Anti-Hallucination Guard setting
    suspend fun saveEnableAntiHallucination(isEnabled: Boolean) {
        context.apiDataStore.edit { preferences -> preferences[ENABLE_ANTI_HALLUCINATION] = isEnabled }
    }

    suspend fun saveEnableFactGuard(isEnabled: Boolean) {
        context.apiDataStore.edit { preferences -> preferences[ENABLE_FACT_GUARD] = isEnabled }
    }

    suspend fun saveEnableOutputDiscipline(isEnabled: Boolean) {
        context.apiDataStore.edit { preferences -> preferences[ENABLE_OUTPUT_DISCIPLINE] = isEnabled }
    }

    suspend fun saveEnableFileGuard(isEnabled: Boolean) {
        context.apiDataStore.edit { preferences -> preferences[ENABLE_FILE_GUARD] = isEnabled }
    }

    fun isAntiHallucinationEnabledBlocking(): Boolean {
        return runBlocking { enableAntiHallucinationFlow.first() }
    }

    fun isFileGuardEnabledBlocking(): Boolean {
        return runBlocking {
            enableAntiHallucinationFlow.first() && enableFileGuardFlow.first()
        }
    }

    fun isSelfEvolutionEnabledBlocking(): Boolean {
        return runBlocking { enableSelfEvolutionFlow.first() }
    }

    suspend fun saveEnableSelfEvolution(isEnabled: Boolean) {
        context.apiDataStore.edit { preferences -> preferences[ENABLE_SELF_EVOLUTION] = isEnabled }
    }
     ...
     */
    suspend fun getBillingModeForProviderModel(providerModel: String): com.cynosure.operit.data.model.BillingMode {
        val preferences = context.apiDataStore.data.first()
        val modeString = preferences[getBillingModeKey(providerModel)]
        return com.cynosure.operit.data.model.BillingMode.fromString(modeString)
    }
     ...
     * @param mode 计费方式
     */
    suspend fun setBillingModeForProviderModel(providerModel: String, mode: com.cynosure.operit.data.model.BillingMode) {
        context.apiDataStore.edit { preferences ->
