...
import com.cynosure.operit.core.tools.system.AccessibilityProviderInstaller
import com.cynosure.operit.provider.IAccessibilityProvider
...
object UIHierarchyManager {
    ...
    // 新的无障碍服务提供者应用的包名
    private const val PROVIDER_PACKAGE_NAME = "com.cynosure.operit.provider"
    // 无障碍服务提供者APK的文件名
    private const val PROVIDER_APK_NAME = "accessibility.apk"
    // 用于绑定的自定义Action，必须与服务提供者应用中的声明一致
    private const val PROVIDER_ACTION = "com.cynosure.operit.provider.IAccessibilityProvider"
    ...
    /**
     * 请求远程服务在指定坐标执行点击。
     * 如果无障碍服务不可用，回退到 shell 命令 (适用于 root 模式)。
     */
    suspend fun performClick(context: Context, x: Int, y: Int): Boolean {
        if (ensureBound(context)) {
            try {
                val result = accessibilityProvider?.performClick(x, y) ?: false
                if (result) return true
            } catch (e: RemoteException) {
                AppLogger.w(TAG, "无障碍点击失败，尝试 shell 回退", e)
            }
        }
        return try {
            ...
            AppLogger.e(TAG, "shell 点击命令执行失败", e)
            false
        }
    }
    ...
    suspend fun performLongPress(context: Context, x: Int, y: Int): Boolean {
        if (ensureBound(context)) {
            try {
                val result = accessibilityProvider?.performLongPress(x, y) ?: false
                if (result) return true
            } catch (e: RemoteException) {
                AppLogger.w(TAG, "无障碍长按失败，尝试 shell 回退", e)
            }
        }
        return try {
            val cmd = "input swipe $x $y $x $y 800"
            val process = Runtime.getRuntime().exec(arrayOf("sh", "-c", cmd))
            process.waitFor()
            process.exitValue() == 0
        } catch (e: Exception) {
            AppLogger.e(TAG, "shell 长按命令执行失败", e)
            false
        }
    }
    ...
    suspend fun performSwipe(context: Context, startX: Int, startY: Int, endX: Int, endY: Int, duration: Long): Boolean {
        if (ensureBound(context)) {
            try {
                val result = accessibilityProvider?.performSwipe(startX, startY, endX, endY, duration) ?: false
                if (result) return true
            } catch (e: RemoteException) {
                AppLogger.w(TAG, "无障碍滑动失败，尝试 shell 回退", e)
            }
        }
        // 回退到 shell input swipe 命令（root 模式）
        return try {
            val cmd = "input swipe $startX $startY $endX $endY ${duration.coerceAtLeast(100)}"
            AppLogger.d(TAG, "执行 shell 滑动: $cmd")
            val process = Runtime.getRuntime().exec(arrayOf("sh", "-c", cmd))
            process.waitFor()
            process.exitValue() == 0
        } catch (e: Exception) {
            AppLogger.e(TAG, "shell 滑动命令执行失败", e)
            false
        }
    }
