package com.cynosure.operit.data.mcp
...
import com.cynosure.operit.data.mcp.plugins.MCPConfigGenerator
...
class MCPRepository(private val context: Context) {
        ...
        try {
            ...
            ZipInputStream(BufferedInputStream(zipFile.inputStream())).use { zipInputStream ->
                ...
                while (entry != null) {
                    ...
                    val outFile = File(targetRoot, entryName).canonicalFile
                    val targetPrefix = targetRoot.path + File.separator
                    if (outFile.path != targetRoot.path && !outFile.path.startsWith(targetPrefix)) {
                        throw IOException("ZIP entry escapes target directory: $entryName")
                    }
    ...
    suspend fun syncBridgeStatus() {
        withContext(Dispatchers.IO) {
            ...
            try {
                val bridge = com.cynosure.operit.data.mcp.plugins.MCPBridge.getInstance(context)
