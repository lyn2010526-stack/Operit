...
fun ChatScreenHeader(
    ...
    val activeCardAvatarUriState = remember { mutableStateOf<String?>(null) }
    LaunchedEffect(activeCharacterCard?.id) {
        val flow = activeCharacterCard?.id?.let {
            userPreferencesManager.getAiAvatarForCharacterCardFlow(it)
        } ?: flowOf(null)
        flow.distinctUntilChanged().collect { uri ->
            activeCardAvatarUriState.value = uri
        }
    }

    val activeGroupAvatarUriState = remember { mutableStateOf<String?>(null) }
    LaunchedEffect(activeCharacterGroup?.id) {
        val flow = activeCharacterGroup?.id?.let {
            userPreferencesManager.getAiAvatarForCharacterGroupFlow(it)
        } ?: flowOf(null)
        flow.distinctUntilChanged().collect { uri ->
            activeGroupAvatarUriState.value = uri
        }
    }
    ...
    val activeGroupFallbackMemberAvatarUriState = remember { mutableStateOf<String?>(null) }
    LaunchedEffect(activeGroupFallbackMemberCardId) {
        val flow = activeGroupFallbackMemberCardId?.let {
            userPreferencesManager.getAiAvatarForCharacterCardFlow(it)
        } ?: flowOf(null)
        flow.distinctUntilChanged().collect { uri ->
            activeGroupFallbackMemberAvatarUriState.value = uri
        }
    }
    val activeCharacterAvatarUri =
        when (activePrompt) {
            is ActivePrompt.CharacterGroup -> activeGroupAvatarUriState.value
                ?: activeGroupFallbackMemberAvatarUriState.value
            is ActivePrompt.CharacterCard -> activeCardAvatarUriState.value
        }
