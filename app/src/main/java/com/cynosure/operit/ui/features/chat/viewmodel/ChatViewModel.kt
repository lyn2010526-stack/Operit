package com.cynosure.operit.ui.features.chat.viewmodel
...
import com.cynosure.operit.ui.features.chat.components.ChatStyle
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.cynosure.operit.api.chat.ChatRuntimeHolder
import com.cynosure.operit.api.chat.ChatRuntimeSlot
import com.cynosure.operit.api.chat.EnhancedAIService
import com.cynosure.operit.core.chat.AIMessageManager
...
import com.cynosure.operit.core.tools.FileOperationData
import com.cynosure.operit.data.collects.ApiProviderConfigs
import com.cynosure.operit.data.model.ApiProviderType
import com.cynosure.operit.data.model.AttachmentInfo
...
import com.cynosure.operit.data.model.ChatMessageLocatorPreview
import com.cynosure.operit.data.model.FunctionType
import com.cynosure.operit.data.model.PromptFunctionType
import com.cynosure.operit.data.model.ToolParameter
import com.cynosure.operit.R
import com.cynosure.operit.ui.features.chat.webview.LocalWebServer
import com.cynosure.operit.ui.floating.FloatingMode
import com.cynosure.operit.ui.permissions.PermissionLevel
...
import com.cynosure.operit.ui.floating.ui.pet.AvatarEmotionManager
import com.cynosure.operit.api.voice.VoiceService
import com.cynosure.operit.api.voice.VoiceServiceFactory
import com.cynosure.operit.data.preferences.SpeechServicesPreferences
...
import com.cynosure.operit.data.model.ActivePrompt
import com.cynosure.operit.ui.features.chat.webview.workspace.WorkspaceBackupManager
import com.cynosure.operit.ui.features.chat.webview.workspace.CommandConfig
import com.cynosure.operit.ui.features.chat.webview.workspace.WorkspaceCommandExecutionState
import com.cynosure.operit.ui.features.chat.webview.workspace.WorkspaceConfigReader
import com.cynosure.operit.ui.features.chat.webview.workspace.WorkspacePreviewRefreshBus
import com.cynosure.operit.ui.features.chat.webview.workspace.WorkspacePreviewRefreshEvent
import com.cynosure.operit.ui.features.chat.webview.workspace.toWorkspaceCommandOutputEntries
import com.cynosure.operit.core.tools.system.Terminal
import com.cynosure.operit.util.TtsReadableContent
import com.cynosure.operit.util.TtsSegmenter
import com.cynosure.operit.ui.features.chat.util.findMentionTokens
import com.cynosure.operit.ui.features.chat.util.findMentionTokenEndingAtCursor
import com.cynosure.operit.ui.features.chat.util.isMentionContinuation
import android.net.Uri
import android.os.Build
import androidx.annotation.RequiresApi
import java.io.File
// 使用 services/core 的 Delegate 类
import com.cynosure.operit.services.core.MessageProcessingDelegate
import com.cynosure.operit.services.core.ChatHistoryDelegate
import com.cynosure.operit.services.core.ApiConfigDelegate
import com.cynosure.operit.services.core.TokenStatisticsDelegate
import com.cynosure.operit.services.core.AttachmentDelegate
import com.cynosure.operit.services.core.MessageCoordinationDelegate
import com.cynosure.operit.data.model.InputProcessingState
import com.cynosure.operit.data.model.WorkspaceRenameResult
import com.cynosure.operit.services.ChatServiceCore
import com.cynosure.operit.services.ChatServiceUiBridge
import com.cynosure.operit.services.EmptyChatServiceUiBridge
import com.cynosure.operit.ui.features.chat.util.MessageImageGenerator
import com.cynosure.operit.ui.features.chat.components.CharacterSelectorTarget
...
class ChatViewModel(private val context: Context) : ViewModel() {
    ...
    companion object {
        ...
        private const val PLAY_ALL_PAGE_SIZE = 40
    }
    ...
    private val speechSessionGeneration = AtomicLong(0)
    private val _isPlayAllActive = MutableStateFlow(false)
    val isPlayAllActive: StateFlow<Boolean> = _isPlayAllActive.asStateFlow()
    private val _hasReadableAiMessages = MutableStateFlow(false)
    val hasReadableAiMessages: StateFlow<Boolean> = _hasReadableAiMessages.asStateFlow()
    ...
    init {
        ...
        setupReadableAiMessageAvailability()
    ...
    private fun setupReadableAiMessageAvailability() {
        viewModelScope.launch {
            combine(chatHistoryDelegate.currentChatId, chatHistoryDelegate.chatHistory) { chatId, _ -> chatId }
                .collect { chatId ->
                    _hasReadableAiMessages.value = chatId != null && hasReadableAiMessage(chatId)
                }
        }
    }

    private suspend fun hasReadableAiMessage(chatId: String): Boolean {
        if (!chatHistoryDelegate.hasReadableAiMessage(chatId)) return false
        val cleanerRegexs = speechServicesPreferences.ttsCleanerRegexsFlow.first()
        var offset = 0
        while (chatHistoryDelegate.currentChatId.value == chatId) {
            val page = chatHistoryDelegate.loadReadableAiMessageContents(
                chatId = chatId,
                offset = offset,
                limit = PLAY_ALL_PAGE_SIZE,
            )
            if (page.any { TtsReadableContent.isReadable(it, cleanerRegexs) }) return true
            if (page.size < PLAY_ALL_PAGE_SIZE) return false
            offset += page.size
        }
        return false
    }
    ...
    val enableAntiHallucination: StateFlow<Boolean> by lazy { apiConfigDelegate.enableAntiHallucination }
    val enableFactGuard: StateFlow<Boolean> by lazy { apiConfigDelegate.enableFactGuard }
    val enableOutputDiscipline: StateFlow<Boolean> by lazy { apiConfigDelegate.enableOutputDiscipline }
    val enableFileGuard: StateFlow<Boolean> by lazy { apiConfigDelegate.enableFileGuard }
    val agentExecutionSnapshots: StateFlow<Map<String, AgentExecutionSnapshot>> =
        AgentExecutionStore.snapshots

    fun pauseAgentTask(taskId: String) {
        AgentExecutionStore.pause(taskId, "用户暂停任务")
    }

    fun resumeAgentTask(taskId: String) {
        AgentExecutionStore.resume(taskId, "用户继续任务")
    }

    fun cancelAgentTask(taskId: String) {
        AgentExecutionStore.cancel(taskId, "用户取消任务")
        cancelCurrentMessage()
    }

    fun toggleAntiHallucination() {
        apiConfigDelegate.toggleAntiHallucination()
    }

    fun toggleFactGuard() = apiConfigDelegate.toggleFactGuard()

    fun toggleOutputDiscipline() = apiConfigDelegate.toggleOutputDiscipline()

    fun toggleFileGuard() = apiConfigDelegate.toggleFileGuard()
    ...
    private fun findLatestChatForCharacterCard(targetCard: com.cynosure.operit.data.model.CharacterCard): ChatHistory? {
    ...
    fun speakMessage(message: String, interrupt: Boolean) {
        ...
        speechPlaybackJob = viewModelScope.launch {
            try {
                speechStopJob?.join()
                speakMessageContent(message, interrupt, generation)
            } catch (e: CancellationException) {
                logSpeechState(
    ...
    private suspend fun speakMessageContent(
        message: String,
        interrupt: Boolean,
        generation: Long,
        resetPauseState: Boolean = true,
    ): Boolean {
        ...
        val cleanerRegexs = speechServicesPreferences.ttsCleanerRegexsFlow.first()
        val cleanMessage = TtsReadableContent.prepare(message, cleanerRegexs)
        if (cleanMessage.isBlank()) return true

        val segments = TtsSegmenter.split(cleanMessage)
        for ((index, segment) in segments.withIndex()) {
            if (generation != speechSessionGeneration.get()) return false
            isSpeechPaused.filter { paused -> !paused }.first()
            ...
            while (_isSpeechPaused.value || currentVoiceService.isSpeaking) {
                if (generation != speechSessionGeneration.get()) return false
                delay(50)
            }
        }
    ...
    /** 连续播放全部对话（仅 AI 消息） */
    fun playAllConversation() {
        val chatId = chatHistoryDelegate.currentChatId.value ?: return
        if (!_hasReadableAiMessages.value) return
        val generation = speechSessionGeneration.incrementAndGet()
        playAllJob?.cancel()
        speechPlaybackJob?.cancel()
        speechPlaybackJob = null
        playAllJob = viewModelScope.launch {
            try {
                speechStopJob?.join()
                _isPlayAllActive.value = true
                _isSpeechPaused.value = false
                var offset = 0
                playback@ while (generation == speechSessionGeneration.get() &&
                    ...
                    for (messageContent in page) {
                        if (generation != speechSessionGeneration.get() ||
                            chatHistoryDelegate.currentChatId.value != chatId
                        ) break
                        if (!speakMessageContent(
                    ...
                    if (page.size < PLAY_ALL_PAGE_SIZE) break
                    offset += page.size
                }
            } catch (e: CancellationException) {
                AppLogger.d(TAG, "playAllConversation cancelled")
            } catch (e: Exception) {
                AppLogger.e(TAG, "播放全部对话失败", e)
            } finally {
                if (generation == speechSessionGeneration.get()) {
                    _isPlayAllActive.value = false
                    _isSpeechSessionActive.value = false
    ...
    fun pauseSpeaking() {
        viewModelScope.launch {
            try {
                ...
                if (success || _isPlayAllActive.value) {
                    _isSpeechSessionActive.value = true
                    cancelSpeechControlsHide("pauseSpeaking.done")
                    logSpeechState("pauseSpeaking.done", "success=$success playAll=${_isPlayAllActive.value}")
                } else {
                    _isSpeechPaused.value = false
        ...
        viewModelScope.launch {
            try {
                ...
                if (success || _isPlayAllActive.value) {
                    _isSpeechPaused.value = false
                    _isSpeechSessionActive.value = true
                    logSpeechState("resumeSpeaking.done", "success=$success playAll=${_isPlayAllActive.value}")
                } else {
