...
class AccessibilityShellExecutor(private val context: Context) : ShellExecutor {
    ...
    override suspend fun startProcess(command: String): ShellProcess {
        return AccessibilityShellProcess(command)
    }
...
private class AccessibilityShellProcess(
    ...
    override val stdout: Flow<String> =
        flowOf("Accessibility service cannot execute shell commands directly")

    override val stderr: Flow<String> =
        flowOf(
            "Command: $command",
            "Accessibility service requires UI automation conversion"
        )
    ...
    override suspend fun waitFor(): Int = -1
}
