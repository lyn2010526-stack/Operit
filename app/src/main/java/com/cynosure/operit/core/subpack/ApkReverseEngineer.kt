package com.cynosure.operit.core.subpack
...
class ApkReverseEngineer(private val context: Context) {
    ...
    /**
     * 将目录中的文件按相对路径覆盖到原版 APK，同路径替换，其他路径新增。
     * 输出文件已移除旧签名并完成 zipalign，可继续交给 [signApk] 签名。
     */
    fun repackageApkWithOverlay(
            inputApk: File,
            outputApk: File,
            overlayDir: File,
            ...
            newVersionCode: String? = null
    ): Boolean {
        var tempUnalignedApk: File? = null
        return try {
            require(inputApk.isFile && inputApk.extension.equals("apk", ignoreCase = true)) {
                "输入文件不是有效的 APK: ${inputApk.absolutePath}"
            }
            require(overlayDir.isDirectory) { "Overlay 目录无效: ${overlayDir.absolutePath}" }
            require(inputApk.canonicalFile != outputApk.canonicalFile) { "输入和输出 APK 路径不能相同" }

            val overlayFiles = collectOverlayFiles(overlayDir)
            outputApk.parentFile?.mkdirs()
            val workingApk =
                    File(outputApk.parentFile, "${outputApk.nameWithoutExtension}_overlay_unaligned.apk")
            tempUnalignedApk = workingApk
            if (workingApk.exists()) workingApk.delete()

            ZipArchiveOutputStream(FileOutputStream(workingApk)).use { zipOut ->
                ZipFile(inputApk).use { zip ->
                    ...
                    while (entries.hasMoreElements()) {
                        ...
                        val entryName = normalizeApkEntryName(entry.name)
                        require(seenEntries.add(entryName)) { "原版 APK 包含重复条目: $entryName" }

                        if (entryName.startsWith("META-INF/")) continue

                        val overlayFile = overlayFiles[entryName]
                        if (overlayFile != null) {
                            val bytes = prepareOverlayBytes(
                                    entryName,
                                    overlayFile.readBytes(),
                                    newPackageName,
                                    newAppName,
                                    newVersionName,
                                    newVersionCode
                            )
                            writeBytesEntry(zipOut, entryName, bytes, overlayFile.lastModified(), entry.method)
                            continue
                        }

                        if (entryName == ANDROID_MANIFEST && hasManifestChanges(
                                        newPackageName,
                                        newAppName,
                                        newVersionName,
                                        newVersionCode
                    ...
                    overlayFiles
                            .filterKeys { it !in seenEntries && !it.startsWith("META-INF/") }
                            .toSortedMap()
                            .forEach { (entryName, file) ->
                                ...
                                writeBytesEntry(zipOut, entryName, bytes, file.lastModified())
                            }
                }
            }

            val aligned = zipalign(workingApk, outputApk, 4)
            workingApk.delete()
            aligned
        } catch (e: Exception) {
            AppLogger.e(TAG, "APK overlay 打包失败", e)
            tempUnalignedApk?.takeIf { it.exists() }?.delete()
            false
        }
    }

    /** 完成 overlay、zipalign 和 APK 签名。 */
    fun repackageAndSignApkWithOverlay(
            inputApk: File,
            outputApk: File,
            overlayDir: File,
            keyStoreFile: File,
        ...
        val alignedApk = File(outputApk.parentFile, "${outputApk.nameWithoutExtension}_aligned_unsigned.apk")
        if (!repackageApkWithOverlay(
                        inputApk,
                        alignedApk,
                        overlayDir,
                        newPackageName,
            ...
            return Pair(false, "APK overlay 打包或对齐失败")
        }
    ...
    private fun collectOverlayFiles(overlayDir: File): Map<String, File> {
        val root = overlayDir.canonicalFile
        val files = linkedMapOf<String, File>()
        overlayDir.walkTopDown().filter { it.isFile }.sortedBy { it.absolutePath }.forEach { file ->
            val canonicalFile = file.canonicalFile
            require(canonicalFile.toPath().startsWith(root.toPath())) {
                "Overlay 文件超出根目录: ${file.absolutePath}"
            }
            val entryName = normalizeApkEntryName(canonicalFile.relativeTo(root).invariantSeparatorsPath)
            require(files.put(entryName, canonicalFile) == null) { "Overlay 包含重复条目: $entryName" }
        }
        return files
    }

    private fun normalizeApkEntryName(entryName: String): String {
        val normalized = entryName.replace('\\', '/').trimStart('/')
        require(normalized.isNotBlank()) { "APK 条目路径为空" }
        require(normalized.split('/').none { it.isEmpty() || it == "." || it == ".." }) {
            "APK 条目路径非法: $entryName"
        }
        return normalized
    }

    private fun hasManifestChanges(
            newPackageName: String?,
            newAppName: String?,
            newVersionName: String?,
            newVersionCode: String?
    ): Boolean =
            newPackageName != null || newAppName != null || newVersionName != null || newVersionCode != null

    private fun prepareOverlayBytes(
            entryName: String,
            bytes: ByteArray,
            newPackageName: String?,
            newAppName: String?,
            ...
            if (entryName == ANDROID_MANIFEST && hasManifestChanges(
                ...
                modifyManifestBytes(bytes, newPackageName, newAppName, newVersionName, newVersionCode)
            } else {
                bytes
            }
