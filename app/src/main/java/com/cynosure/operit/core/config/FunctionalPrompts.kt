...
import com.cynosure.operit.core.avatar.common.state.AvatarCustomMoodDefinition
import com.cynosure.operit.core.avatar.common.state.AvatarMoodTypes
...
You are a group collaboration planner. Return ONLY valid JSON.
Task: plan the collaboration workflow for this turn. Members can execute tasks, give feedback, review, and correct each other.

Output schema:
{"rounds":[[{"id":"<memberId>","speak":true,"task":"<task description>","feedbackTo":"<memberId>","review":"<approve|revision>","correction":["<issue>"]}]]}

Fields:
- "id": member ID (required)
- "speak": whether this member should respond (default true)
- "task": specific task assigned to this member (e.g. "implement feature X", "review code")
- "feedbackTo": if this member is providing feedback/correction to another member, set their ID
- "review": set to "approve" if task is confirmed complete, "revision" if changes needed
- "correction": list of specific issues found that need fixing

Rules:
- Each round is an array of members who should speak in that round.
- Plan multiple rounds for task -> feedback -> correction -> completion cycles.
- For simple responses, use a single round with one or more members.
- Members can @mention other members by their name or ID to request specific actions.
- When a member completes a task, another member should review and provide feedback.
- If review finds issues, the original member should be asked to revise in a subsequent round.
- The cycle continues until all tasks are approved or max rounds reached.
- Members may be omitted via speak=false to skip them.
- If no one should respond, return {"rounds":[[]]}.
- Use ONLY the provided member IDs.
- Maximum {maxRounds} rounds.
    """

    const val GROUP_ROLE_RESPONSE_PLANNER_PROMPT_CN = """
你是群组协作规划器。只返回有效的 JSON。
任务：规划本轮协作流程。成员可以执行任务、反馈、审核和互相纠错。

输出格式：
{"rounds":[[{"id":"<成员ID>","speak":true,"task":"<任务描述>","feedbackTo":"<成员ID>","review":"<approve|revision>","correction":["<问题>"]}]]}

字段说明：
- "id"：成员 ID（必需）
- "speak"：该成员是否应发言（默认 true）
- "task"：分配给该成员的具体任务（如"实现功能 X"、"审查代码"）
- "feedbackTo"：如果该成员正在向另一成员提供反馈/纠错，设置其 ID
- "review"：设为 "approve" 表示任务确认完成，"revision" 表示需要修改
- "correction"：发现的具体问题列表，需要被指派人修复

规则：
- 每一轮（round）是一个数组，包含该轮应发言的成员。
- 规划多轮来实现 任务 -> 反馈 -> 纠错 -> 完成 的闭环。
- 对于简单回应，使用单轮包含一个或多个成员即可。
- 成员可以通过 @成员名 或 @成员ID 来定向请求某个成员执行特定操作。
- 成员完成任务后，应由另一成员审核并提供反馈。
- 如果审核发现问题，应让原成员在后续轮次中修改。
- 循环持续到所有任务被批准或达到最大轮数。
- 可设置 speak=false 跳过某个成员。
- 如果无人应回应，返回 {"rounds":[[]]}。
- 只使用提供的成员 ID。
- 最大 {maxRounds} 轮。
    """

    fun groupRoleResponsePlannerPrompt(useEnglish: Boolean, maxRounds: Int = 5): String {
        val template = if (useEnglish) GROUP_ROLE_RESPONSE_PLANNER_PROMPT else GROUP_ROLE_RESPONSE_PLANNER_PROMPT_CN
        return template.replace("{maxRounds}", maxRounds.toString())
    }
    ...
    fun buildGroupRoleResponsePlannerPrompt(
        ...
        val basePrompt = groupRoleResponsePlannerPrompt(useEnglish, maxRounds)
        return buildString {
            append(basePrompt)
            appendLine()
            if (groupRules.isNotBlank()) {
                if (useEnglish) {
                    appendLine("Group operating rules:")
                    appendLine(groupRules)
                    appendLine()
                } else {
                    appendLine("群组运行规则：")
                    appendLine(groupRules)
    ...
    fun buildGroupRoleResponsePlannerPromptWithRoles(
        memberLines: String,
        memberRoles: String,
        userText: String,
        useEnglish: Boolean,
        ...
        return buildString {
            ...
            if (useEnglish) {
                ...
                appendLine(memberLines.ifBlank { "(none)" })
                appendLine()
                appendLine("Member roles:")
                appendLine(memberRoles.ifBlank { "(none)" })
                appendLine()
                appendLine("User message:")
                appendLine(userText.ifBlank { "(user sent attachments or empty text)" })
            } else {
                ...
                appendLine()
                appendLine("成员职责：")
                appendLine(memberRoles.ifBlank { "（无）" })
                appendLine()
                appendLine("用户消息：")
                appendLine(userText.ifBlank { "（用户发送了附件或空文本）" })
            }
    ...
    // 单轮规划提示词
    const val GROUP_ROLE_RESPONSE_PLANNER_NEXT_ROUND_PROMPT = """
You are a group collaboration planner that produces the next round. Return only valid JSON.

Output format:
{"round": [{"id":"<memberID>","speak":true,"task":"<task description>","feedbackTo":"<memberID>","review":"<approve|revision>","correction":["<issue>"]}]}

Fields:
- "id": member ID (required)
- "speak": whether this member should respond (default true)
- "task": specific task assigned (e.g. "implement feature X", "review code")
- "feedbackTo": if providing feedback to another member, set their ID
- "review": "approve" if task confirmed complete, "revision" if changes needed
- "correction": list of specific issues found

Rules:
- Return exactly ONE round with the members who should speak next.
- Use the conversation timeline and task status to decide what should happen next.
- If a task is assigned but not yet reviewed, assign a reviewer.
- If review found issues, assign the original member to revise.
- If all tasks are approved, return {"round": []}.
- Members can be omitted via speak=false to skip them.
- Maximum {maxRounds} rounds total, plan accordingly.
"""

    const val GROUP_ROLE_RESPONSE_PLANNER_NEXT_ROUND_PROMPT_CN = """
你是群组协作规划器，规划下一个发言轮次。只返回有效的 JSON。

输出格式：
{"round": [{"id":"<成员ID>","speak":true,"task":"<任务描述>","feedbackTo":"<成员ID>","review":"<approve|revision>","correction":["<问题>"]}]}
...
"""
    ...
    fun buildGroupRoleResponsePlannerNextRoundPrompt(
        memberLines: String,
        memberRoles: String,
        userText: String,
        useEnglish: Boolean,
        maxRounds: Int = 5,
        isSelfCorrectionEnabled: Boolean = true,
        groupRules: String = "",
        timeline: String = ""
    ): String {
        val template = if (useEnglish) GROUP_ROLE_RESPONSE_PLANNER_NEXT_ROUND_PROMPT
            else GROUP_ROLE_RESPONSE_PLANNER_NEXT_ROUND_PROMPT_CN
        val basePrompt = template.replace("{maxRounds}", maxRounds.toString())
        val adjustedPrompt = if (!isSelfCorrectionEnabled) {
            val lines = basePrompt.lines()
            val excludeEn = setOf(
                "- \"feedbackTo\":",
                "- \"review\":",
                "- \"correction\":",
            ...
            val excludes = if (useEnglish) excludeEn else excludeCn
            lines.filter { line -> excludes.none { ex -> line.contains(ex) } }
                .joinToString("\n")
        } else basePrompt
        return buildString {
            append(adjustedPrompt)
            appendLine()
            if (groupRules.isNotBlank()) {
                if (useEnglish) {
                    appendLine("Group operating rules:")
            ...
            if (useEnglish) {
                ...
                appendLine(memberLines.ifBlank { "(none)" })
                appendLine()
                if (memberRoles.isNotBlank()) {
                    appendLine("Member roles:")
                    appendLine(memberRoles)
                    appendLine()
                }
                appendLine("Conversation timeline:")
                appendLine(timeline.ifBlank { "(no previous context)" })
                appendLine()
                appendLine("User message:")
                appendLine(userText.ifBlank { "(user sent attachments or empty text)" })
            } else {
                ...
                appendLine("对话进展：")
                appendLine(timeline.ifBlank { "（无前置上下文）" })
                appendLine()
                appendLine("用户消息：")
                appendLine(userText.ifBlank { "（用户发送了附件或空文本）" })
            }
