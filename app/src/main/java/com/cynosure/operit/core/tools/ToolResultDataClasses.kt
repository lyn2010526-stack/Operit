...
data class WorkflowDetailResultData(
    ...
    val name: String,
    val description: String,
    val nodes: List<com.cynosure.operit.data.model.WorkflowNode>,
    val connections: List<com.cynosure.operit.data.model.WorkflowNodeConnection>,
    val enabled: Boolean,
    val createdAt: Long,
    val updatedAt: Long,
    val lastExecutionTime: Long? = null,
    ...
    override fun toString(): String {
        ...
        nodes.forEach { node ->
            when (node) {
                is com.cynosure.operit.data.model.TriggerNode -> {
                ...
                is com.cynosure.operit.data.model.ExecuteNode -> {
                ...
                is com.cynosure.operit.data.model.ConditionNode -> {
                ...
                is com.cynosure.operit.data.model.LogicNode -> {
                ...
                is com.cynosure.operit.data.model.ExtractNode -> {
...
data class GitStatusData(
    ...
    override fun toString(): String = buildString {
        ...
        if (entries.isEmpty()) append("(clean working tree)")
        else entries.forEach {
            append("${it.indexStatus}${it.workTreeStatus} ${it.file}\n")
        }
    }
}
...
data class GitDiffData(
    ...
    override fun toString(): String = if (truncated) {
        "$diff\n\n[diff truncated; showing first ${diff.length} of $totalSize bytes]"
    } else diff
}
...
@Serializable
data class GitLogData(
    val entries: List<GitLogEntry>,
    val repoPath: String
) : ToolResultData() {
    override fun toString(): String = buildString {
        entries.forEach { append("${it.hash} ${it.message} (${it.author}, ${it.relativeDate})\n") }
    }
}

@Serializable
data class GitBlameEntry(
    val hash: String,
    val author: String,
...
@Serializable
data class GitBlameData(
    val entries: List<GitBlameEntry>,
    val file: String,
    val repoPath: String
) : ToolResultData() {
    override fun toString(): String = buildString {
        entries.forEach {
            append("${it.hash} (${it.author}) L${it.lineNumber}: ${it.codeLine}\n")
        }
    }
}
