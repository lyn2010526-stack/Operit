...
import com.cynosure.operit.core.chat.hooks.PromptHookContext
import com.cynosure.operit.core.chat.hooks.PromptHookRegistry
import com.cynosure.operit.data.model.SystemToolPromptCategory
...
""".trimEnd()
    ...
    val fileSystemTools = SystemToolPromptCategory(
        ...
        tools = listOf(
            ...
            ToolPrompt(
                name = "read_file",
                description = "Read the content of a file. For image files (jpg, jpeg, png, gif, bmp), it automatically extracts text using OCR. Use offset and limit to read a specific line range. Use type=info to just get file metadata.",
                parametersStructured = listOf(
                    ToolParameterSchema(
                        name = "path",
                        type = "string",
                    ...
                    ToolParameterSchema(
                        name = "offset",
                        type = "integer",
                        description = "optional, starting line number (1-indexed). Use with limit for partial reads.",
                        required = false
                    ),
                    ToolParameterSchema(
                        name = "limit",
                        type = "integer",
                        description = "optional, max lines to read from offset. Use with offset for partial reads.",
                        required = false
                    ),
                    ToolParameterSchema(
                        name = "type",
                        type = "string",
                        description = "optional, 'info' to get file metadata (size, lines) without content",
                        required = false
                    ),
                    ToolParameterSchema(
                        name = "intent",
            ...
            // read_file_part merged into read_file above. Use read_file with offset/limit.
            ToolPrompt(
                name = "create_file",
            ...
            ToolPrompt(
                name = "grep_context",
                description = "Based on intent/context understanding, search for related content. Supports two modes: 1) Directory mode: when path is a directory, find the most relevant files. 2) File mode: when path is a file, find the most relevant code sections within the file. Uses semantic relevance scoring.",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Directory or file path", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "Optional, same as read_file's environment", required = false),
                    ToolParameterSchema(name = "intent", type = "string", description = "The intent or context description string", required = true),
                    ToolParameterSchema(name = "file_pattern", type = "string", description = "File filter in directory mode", required = false, default = "\"*\""),
                    ToolParameterSchema(name = "max_results", type = "integer", description = "Maximum number of items to return", required = false, default = "10")
                )
            ),
            ToolPrompt(
                name = "find_references",
                description = "Find all references to a symbol/string across files in a directory. Returns file paths with line numbers where the symbol appears. Use for cross-file reference tracking, API usage discovery, and impact analysis.",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Directory path to search in", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "Optional, same as read_file's environment", required = false),
                    ToolParameterSchema(name = "symbol", type = "string", description = "The symbol or string to search for (e.g. function name, class name, variable)", required = true),
                    ToolParameterSchema(name = "file_pattern", type = "string", description = "File filter pattern", required = false, default = "\"*\""),
                    ToolParameterSchema(name = "max_results", type = "integer", description = "Maximum number of results", required = false, default = "50"),
                    ToolParameterSchema(name = "case_sensitive", type = "boolean", description = "Boolean, case-sensitive search for symbols with mixedCase", required = false, default = "false")
                )
            ),
            ToolPrompt(
                name = "download_file",
    ...
    val fileSystemToolsCn = SystemToolPromptCategory(
        ...
        tools = listOf(
            ...
            ToolPrompt(
                name = "find_references",
                description = "在目录中跨文件搜索符号/字符串的所有引用位置。返回文件路径和行号。用于跨文件引用追踪、API 使用发现、影响范围分析。",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "要搜索的目录路径", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "可选，同 read_file 的 environment", required = false),
                    ToolParameterSchema(name = "symbol", type = "string", description = "要搜索的符号或字符串，如函数名、类名、变量名", required = true),
                    ToolParameterSchema(name = "file_pattern", type = "string", description = "文件过滤模式", required = false, default = "\"*\""),
                    ToolParameterSchema(name = "max_results", type = "integer", description = "最大结果数", required = false, default = "50"),
                    ToolParameterSchema(name = "case_sensitive", type = "boolean", description = "布尔值，针对 mixedCase 符号启用大小写敏感搜索", required = false, default = "false")
                )
            ),
            ToolPrompt(
                name = "download_file",
    ...
    // ==================== Git 代码管理工具 ====================
    val codeManagementTools = SystemToolPromptCategory(
        categoryName = "Git & Code Management Tools",
        tools = listOf(
            ToolPrompt(
                name = "git_status",
                description = "Show the working tree status of a git repository. Returns the current branch, staged changes (index), and unstaged changes (work tree). Use after making file changes to verify the state before committing.",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Path to the git repository (any directory inside the repo)", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "Optional, same as read_file's environment", required = false)
                )
            ),
            ToolPrompt(
                name = "git_diff",
                description = "Show changes between commits, the working tree, and the index. Returns unified diff output. Use before committing to review exact changes, or to understand what modifications have been made.",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Path to the git repository", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "Optional, same as read_file's environment", required = false),
                    ToolParameterSchema(name = "staged", type = "boolean", description = "Show staged (cached) changes instead of unstaged", required = false, default = "false"),
                    ToolParameterSchema(name = "file", type = "string", description = "Limit diff to a specific file path", required = false)
                )
            ),
            ToolPrompt(
                name = "git_log",
                description = "Show the commit history for a git repository. Returns recent commits with hash, message, author, and relative date. Use to understand recent development activity and find specific commits.",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Path to the git repository", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "Optional, same as read_file's environment", required = false),
                    ToolParameterSchema(name = "count", type = "integer", description = "Number of recent commits to show (max 50)", required = false, default = "20"),
                    ToolParameterSchema(name = "file", type = "string", description = "Limit log to commits affecting a specific file", required = false)
                )
            ),
            ToolPrompt(
                name = "git_blame",
                description = "Show what revision and author last modified each line of a file. Use to find who made specific changes and when, for debugging or code review context.",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Path to the git repository", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "Optional, same as read_file's environment", required = false),
                    ToolParameterSchema(name = "file", type = "string", description = "File path relative to the repository root", required = true),
                    ToolParameterSchema(name = "start_line", type = "integer", description = "Optional, start line number", required = false),
                    ToolParameterSchema(name = "end_line", type = "integer", description = "Optional, end line number", required = false)
                )
            )
        )
    )

    val codeManagementToolsCn = SystemToolPromptCategory(
        categoryName = "Git 与代码管理工具",
        tools = listOf(
            ToolPrompt(
                name = "git_status",
                description = "显示 Git 仓库的工作树状态。返回当前分支、暂存区变更（index）和未暂存变更（work tree）。在文件修改后使用，以便在提交前验证状态。",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Git 仓库路径（仓库内的任意目录即可）", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "可选，同 read_file 的 environment", required = false)
                )
            ),
            ToolPrompt(
                name = "git_diff",
                description = "显示提交之间、工作树与暂存区之间的变更。返回统一 diff 格式输出。在提交前使用以审查具体变更内容。",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Git 仓库路径", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "可选，同 read_file 的 environment", required = false),
                    ToolParameterSchema(name = "staged", type = "boolean", description = "显示已暂存（cached）的变更而非未暂存的变更", required = false, default = "false"),
                    ToolParameterSchema(name = "file", type = "string", description = "限定 diff 到某个文件路径", required = false)
                )
            ),
            ToolPrompt(
                name = "git_log",
                description = "显示 Git 仓库的提交历史。返回最近提交的 hash、消息、作者和相对时间。用于了解近期开发动态和查找特定提交。",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Git 仓库路径", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "可选，同 read_file 的 environment", required = false),
                    ToolParameterSchema(name = "count", type = "integer", description = "显示最近多少条提交（最多 50）", required = false, default = "20"),
                    ToolParameterSchema(name = "file", type = "string", description = "限定日志到影响特定文件的提交", required = false)
                )
            ),
            ToolPrompt(
                name = "git_blame",
                description = "显示文件中每一行最后是由哪个版本和作者修改的。用于在调试或代码审查时追溯特定修改的来源。",
                parametersStructured = listOf(
                    ToolParameterSchema(name = "path", type = "string", description = "Git 仓库路径", required = true),
                    ToolParameterSchema(name = "environment", type = "string", description = "可选，同 read_file 的 environment", required = false),
                    ToolParameterSchema(name = "file", type = "string", description = "相对于仓库根目录的文件路径", required = true),
                    ToolParameterSchema(name = "start_line", type = "integer", description = "可选，起始行号", required = false),
                    ToolParameterSchema(name = "end_line", type = "integer", description = "可选，结束行号", required = false)
                )
            )
        )
    )
