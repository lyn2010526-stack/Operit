...
import com.cynosure.operit.data.model.WorkflowExecutionLogEntry
import com.cynosure.operit.data.model.WorkflowExecutionRecord
import com.cynosure.operit.data.model.WorkflowLogLevel
...
class WorkflowExecutor(private val context: Context) {
    ...
    suspend fun executeWorkflow(
        ...
        val nodeResults: MutableMap<String, NodeExecutionState> = Collections.synchronizedMap(mutableMapOf())
        ...
        try {
            ...
            for (triggerNode in triggerNodes) {
                ...
                setNodeState(triggerNode.id, NodeExecutionState.Success(triggerPayload), nodeResults, onNodeStateChange)
            }
    ...
    private suspend fun executeTopologicalOrder(
        ...
        while (queue.isNotEmpty()) {
            ...
            if (!shouldExecute) {
                ...
                setNodeState(node.id, NodeExecutionState.Skipped(skipReason), nodeResults, onNodeStateChange)
    ...
    private suspend fun executeNode(
        ...
        if (node is TriggerNode) {
            val triggerPayload = org.json.JSONObject(triggerExtras).toString()
            setNodeState(node.id, NodeExecutionState.Success(triggerPayload), nodeResults, onNodeStateChange)
            return true
        }

        if (node is ConditionNode) {
            setNodeState(node.id, NodeExecutionState.Running, nodeResults, onNodeStateChange)
            ...
            return try {
                ...
                setNodeState(node.id, NodeExecutionState.Success(result), nodeResults, onNodeStateChange)
                true
            } catch (e: CancellationException) {
                ...
                setNodeState(node.id, NodeExecutionState.Failed(errorMsg), nodeResults, onNodeStateChange)
                false
            }
        }
        ...
        if (node is ExtractNode) {
            ...
            return try {
                ...
                setNodeState(node.id, NodeExecutionState.Success(extracted), nodeResults, onNodeStateChange)
                true
            } catch (e: CancellationException) {
                throwCancellation(node, runLogger, e)
            } catch (e: Exception) {
        ...
        try {
            ...
            if (result.success) {
                ...
                setNodeState(node.id, NodeExecutionState.Success(resultMessage), nodeResults, onNodeStateChange)
                return true
            } else {
