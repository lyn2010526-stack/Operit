...
import com.cynosure.operit.core.tools.defaultTool.ToolGetter
import com.cynosure.operit.core.tools.defaultTool.standard.StandardGitTools
...
import com.cynosure.operit.data.preferences.ResolvedCharacterCardToolAccess
import com.cynosure.operit.integrations.tasker.triggerAIAgentAction
import com.cynosure.operit.services.FloatingChatService
import com.cynosure.operit.ui.common.displays.VirtualDisplayOverlay
...
fun registerAllTools(handler: AIToolHandler, context: Context) {
    ...
    // 音乐播放工具
    // Removed music playback tools (8 tools): music_play, music_play_queue, music_pause,
    // music_resume, music_stop, music_seek, music_set_volume, music_status.
    // These are rarely needed in AI assistant context. Remove comment if needed.
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                val path = tool.parameters.find { it.name == "path" }?.value ?: ""
                val environment = tool.parameters.find { it.name == "environment" }?.value
                val hasOffset = tool.parameters.any { it.name == "offset" && it.value.isNotBlank() }
                val hasLimit = tool.parameters.any { it.name == "limit" && it.value.isNotBlank() }
                val envInfo = formatEnvInfo(environment)
                if (hasOffset || hasLimit) {
                    val offset = tool.parameters.find { it.name == "offset" }?.value ?: "1"
                    val limit = tool.parameters.find { it.name == "limit" }?.value
                    if (limit != null) {
                        s(R.string.toolreg_read_file_part_range_lines, offset, limit) + " $path$envInfo"
                    } else {
                        s(R.string.toolreg_read_file_part_range_from, offset) + " $path$envInfo"
                    }
                } else {
            ...
            executor = { tool ->
                runBlocking(Dispatchers.IO) {
                    ...
                    val hasLimit = tool.parameters.any { it.name == "limit" && it.value.isNotBlank() }
                    if (hasOffset || hasLimit) {
                        fileSystemTools.readFilePart(tool)
                    } else {
    ...
    // read_file_part and read_file_full merged into read_file above.
    // Use read_file with optional offset/limit parameters instead.
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                ...
                val envInfo = formatEnvInfo(tool.parameters.find { it.name == "environment" }?.value)
                "Find all references to \"$symbol\" in $path$envInfo"
            },
            executor = { tool ->
                runBlocking(Dispatchers.IO) { fileSystemTools.findReferences(tool) }
            }
    )
    ...
    // Removed bluetooth tools (19 tools): request_bluetooth_permission, get_bluetooth_state,
    // request_enable_bluetooth, list_bluetooth_bonded_devices, scan_bluetooth_devices,
    // bluetooth_connect, bluetooth_listen, bluetooth_accept, bluetooth_send, bluetooth_read,
    // bluetooth_send_and_read, bluetooth_close, bluetooth_ble_connect,
    // bluetooth_ble_discover_services, bluetooth_ble_read_characteristic,
    // bluetooth_ble_write_characteristic, bluetooth_ble_write_and_read_characteristic,
    // bluetooth_ble_subscribe_characteristic, bluetooth_ble_read_notifications.
    // These are rarely needed in AI assistant context. Remove comment if needed.
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                val path = tool.parameters.find { it.name == "path" }?.value ?: ""
                val envInfo = formatEnvInfo(tool.parameters.find { it.name == "environment" }?.value)
                "Show git status for $path$envInfo"
            },
            executor = { tool ->
                runBlocking(Dispatchers.IO) { gitTools.gitStatus(tool) }
            }
    )
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                ...
                val envInfo = formatEnvInfo(tool.parameters.find { it.name == "environment" }?.value)
                val mode = if (staged) " (staged)" else ""
                "Show git diff for $path$mode$envInfo"
            },
            executor = { tool ->
                runBlocking(Dispatchers.IO) { gitTools.gitDiff(tool) }
            }
    )
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                val path = tool.parameters.find { it.name == "path" }?.value ?: ""
                val count = tool.parameters.find { it.name == "count" }?.value ?: "20"
                val envInfo = formatEnvInfo(tool.parameters.find { it.name == "environment" }?.value)
                "Show git log (last $count commits) for $path$envInfo"
            },
            executor = { tool ->
                runBlocking(Dispatchers.IO) { gitTools.gitLog(tool) }
            }
    )
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                val path = tool.parameters.find { it.name == "path" }?.value ?: ""
                val file = tool.parameters.find { it.name == "file" }?.value ?: ""
                val envInfo = formatEnvInfo(tool.parameters.find { it.name == "environment" }?.value)
                "Show git blame for $file$envInfo"
            },
            executor = { tool ->
                runBlocking(Dispatchers.IO) { gitTools.gitBlame(tool) }
            }
    )
    ...
    handler.registerTool(
        ...
        descriptionGenerator = { tool ->
            val title = tool.parameters.find { it.name == "title" }?.value ?: ""
            val count = tool.parameters.find { it.name == "questions" }?.let {
                try { JSONArray(it.value).length() } catch (_: Exception) { 0 }
            } ?: 0
            "Show an input form to the user with $count question(s)${if (title.isNotBlank()) ": $title" else ""}"
        },
        executor = CollectUserInputExecutor()
    )
}
