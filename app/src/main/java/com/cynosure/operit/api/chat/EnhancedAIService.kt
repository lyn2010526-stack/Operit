package com.cynosure.operit.api.chat
...
import com.cynosure.operit.api.chat.enhance.ConversationMarkupManager
import com.cynosure.operit.api.chat.enhance.ConversationRoundManager
import com.cynosure.operit.api.chat.enhance.ConversationService
import com.cynosure.operit.api.chat.enhance.FileBindingService
import com.cynosure.operit.api.chat.enhance.MultiServiceManager
import com.cynosure.operit.api.chat.enhance.ToolExecutionManager
import com.cynosure.operit.api.chat.llmprovider.AIService
import com.cynosure.operit.core.chat.logMessageTiming
import com.cynosure.operit.core.chat.messageTimingNow
...
import com.cynosure.operit.core.chat.hooks.appendUserTurnIfMissing
import com.cynosure.operit.core.chat.hooks.buildActivePromptHookMetadata
import com.cynosure.operit.core.chat.hooks.mergeAdjacentTurns
import com.cynosure.operit.core.chat.hooks.toPromptTurns
import com.cynosure.operit.core.chat.hooks.toRoleContentPairs
import com.cynosure.operit.core.application.ActivityLifecycleManager
...
import com.cynosure.operit.core.tools.climode.CliToolModeSupport
import com.cynosure.operit.core.tools.climode.ToolExposureMode
...
import com.cynosure.operit.data.model.ModelConfigData
...
import com.cynosure.operit.core.evolution.SelfEvolutionEngine
import com.cynosure.operit.core.evolution.ValidationRunner
import com.cynosure.operit.data.preferences.ApiPreferences
import com.cynosure.operit.data.preferences.ExternalHttpApiPreferences
import com.cynosure.operit.data.preferences.WakeWordPreferences
import com.cynosure.operit.util.stream.MutableSharedStream
import com.cynosure.operit.util.stream.Stream
import com.cynosure.operit.util.stream.StreamCollector
import com.cynosure.operit.util.stream.TextStreamEvent
import com.cynosure.operit.util.stream.TextStreamEventCarrier
import com.cynosure.operit.util.stream.TextStreamEventType
import com.cynosure.operit.util.stream.TextStreamRevisionTracker
import com.cynosure.operit.util.stream.withEventChannel
import com.cynosure.operit.util.stream.plugins.StreamXmlPlugin
...
import com.cynosure.operit.data.repository.CustomEmojiRepository
import com.cynosure.operit.data.preferences.CharacterCardManager
import com.cynosure.operit.data.preferences.CharacterCardToolAccessResolver
...
import com.cynosure.operit.core.config.SystemToolPrompts
...
class EnhancedAIService private constructor(private val context: Context) {
     ...
     * @param newState 新的状态
     */
    fun setInputProcessingState(newState: com.cynosure.operit.data.model.InputProcessingState) {
    ...
    private data class MessageExecutionContext(
        ...
        val toolValidationInputs: MutableList<ValidationRunner.ValidationInput> = mutableListOf(),
    ...
    init {
        com.cynosure.operit.api.chat.library.MemoryLibrary.initialize(context)
        initScope.launch {
            runCatching {
                ensureInitialized()
            }.onFailure { e ->
    ...
    private suspend fun getModelParametersForFunction(
        ...
        chatModelConfigIdOverride: String? = null,
        chatModelIndexOverride: Int? = null
    ): List<com.cynosure.operit.data.model.ModelParameter<*>> {
        ensureInitialized()
        val overrideConfigId = chatModelConfigIdOverride?.takeIf { it.isNotBlank() }
        return if (functionType == FunctionType.CHAT && overrideConfigId != null) {
    ...
    private suspend fun processStreamCompletion(
        ...
        try {
            ...
            // 防幻觉后置检测：当开关打开时，对完整输出做 OutputFirewall + HallucinationCheck
            val antiHallucinationEnabled = apiPreferences.enableAntiHallucinationFlow.first()
            if (antiHallucinationEnabled && content.isNotEmpty()) {
                val guardResult = AntiHallucinationGuard.fullCheck(
                    text = content,
                    checkOutputDiscipline = apiPreferences.enableOutputDisciplineFlow.first(),
                    checkFacts = apiPreferences.enableFactGuardFlow.first()
                )
                if (!guardResult.overallPass) {
                    AppLogger.w(TAG, "防幻觉检测发现违规: ${guardResult.summary}")
                    val warningText = ConversationMarkupManager.createWarningStatus(
                        this@EnhancedAIService.context.getString(
                            R.string.anti_hallucination_violation_detected,
                            guardResult.summary
                        )
                    )
                    context.roundManager.appendContent("\n$warningText")
                    collector.emit(warningText)
                    context.conversationHistory.add(
                        PromptTurn(kind = PromptTurnKind.TOOL_RESULT, content = warningText)
                    )
                    handleToolInvocation(
                        ...
                        stream = stream,
                        enableGroupOrchestrationHint = enableGroupOrchestrationHint,
                        toolResultOverrideMessage = warningText,
    ...
    private suspend fun finalizeAssistantResponse(
        ...
        // 自进化引擎：任务完成时触发复盘和固化
        runCatching {
            val userMessage = context.conversationHistory
                .lastOrNull { it.kind == com.cynosure.operit.core.chat.hooks.PromptTurnKind.USER }
                ?.content ?: ""
            SelfEvolutionEngine.onTaskCompleted(
                context = this@EnhancedAIService.context,
                info = SelfEvolutionEngine.TaskCompletionInfo(
                    chatId = chatId ?: "",
                    userMessage = userMessage,
                    assistantReply = content,
                    toolsUsed = context.toolsUsed.distinct(),
                    success = content.isNotBlank(),
                    toolValidationInputs = context.toolValidationInputs.toList()
                )
            )
        }
    }
    ...
    private suspend fun handleToolInvocation(
        ...
        val processToolJob = toolProcessingScope.launch {
            ...
            toolInvocations.forEach { invocation ->
                context.toolsUsed.add(invocation.tool.name)
            }
            val remainingResults = allToolResults.toMutableList()
            toolInvocations.forEach { invocation ->
                val resultIndex = remainingResults.indexOfFirst {
                    it.toolName == invocation.tool.name ||
                        it.toolName == invocation.tool.name.substringAfter(':')
                }
                if (resultIndex >= 0 && ValidationRunner.supports(invocation.tool.name)) {
                    val result = remainingResults.removeAt(resultIndex)
                    context.toolValidationInputs.add(
                        ValidationRunner.ValidationInput(
                            toolName = invocation.tool.name,
                            toolParams = invocation.tool.parameters.associate { it.name to it.value },
    ...
    private fun serializePromptHookModelParameters(
        modelParameters: List<com.cynosure.operit.data.model.ModelParameter<*>>
    ): List<Map<String, Any?>> {
    ...
    fun saveConversationToMemoryAsync(
        ...
        toolProcessingScope.launch {
            try {
                ...
                com.cynosure.operit.api.chat.library.MemoryLibrary.saveMemoryAsync(
                    context = context,
                    toolHandler = toolHandler,
