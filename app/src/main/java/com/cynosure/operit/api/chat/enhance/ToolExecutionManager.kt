...
import com.cynosure.operit.core.tools.AIToolHookDecision
import com.cynosure.operit.core.tools.StringResultData
import com.cynosure.operit.core.tools.ToolExecutor
...
object ToolExecutionManager {
    ...
    private const val TOOL_EXECUTION_OVERALL_TIMEOUT_MS = 600_000L
    ...
    private fun isInvocationAllowedForRoleCard(
        invocation: ToolInvocation,
        roleCardToolAccess: com.cynosure.operit.data.preferences.ResolvedCharacterCardToolAccess
    ): Boolean {
    ...
    suspend fun executeInvocations(
        ...
        val parallelizableToolNames = setOf(
            "list_files", "read_file", "file_exists",
            "find_files", "file_info", "grep_code", "calculate", "ffmpeg_info",
            "visit_web", "download_file", "find_references", "git_status", "git_log"
        )
        ...
        // 等待所有并行任务完成。超时后主动取消仍在运行的子任务，确保 coroutineScope 可退出。
        if (parallelJobs.isNotEmpty()) {
            val completed = withTimeoutOrNull(TOOL_EXECUTION_OVERALL_TIMEOUT_MS) {
            ...
            if (!completed) {
                ...
                parallelInvocations.forEach { invocation ->
                    executionResults.putIfAbsent(
                        invocation,
                        ToolResult(
                            toolName = resolveDisplayToolName(invocation.tool),
                            success = false,
                            result = StringResultData(""),
                            error =
                                "Parallel tool batch timed out after " +
                                    "${TOOL_EXECUTION_OVERALL_TIMEOUT_MS / 1000}s"
                        )
                    )
                }
            }
    ...
    private suspend fun executeAndEmitTool(
        ...
        return withContext(toolRuntimeContextThreadLocal.asContextElement(runtimeContext)) {
            try {
                ...
                // 为此调用聚合最终结果
                if (timedOut) {
                    val partialResult = collectedResults.joinToString("\n") { result ->
                        if (result.success) result.result.toString()
                        else "Step error: ${result.error ?: "Unknown error"}"
                    }.trim()
                    val timeoutResultObj =
                        ToolResult(
                            toolName = displayToolName,
                            success = false,
                            result = StringResultData(partialResult),
                            error = "Tool execution timed out after ${TOOL_EXECUTION_TIMEOUT_MS / 1000}s"
                        )
                    val timeoutContent =
                        ConversationMarkupManager.formatToolResultForMessage(timeoutResultObj)
                    collector.emit(ensureEndsWithNewline(timeoutContent))
                    toolHandler.notifyToolExecutionResult(invocation.tool, timeoutResultObj)
                    return@withContext timeoutResultObj
                }
