...
import com.cynosure.operit.core.chat.hooks.SummaryHookContext
import com.cynosure.operit.core.chat.hooks.SummaryHookRegistry
...
import com.cynosure.operit.core.config.SystemPromptConfig
...
import com.cynosure.operit.core.tools.UIPageResultData
import com.cynosure.operit.core.tools.SimplifiedUINode
...
import com.cynosure.operit.core.avatar.impl.factory.AvatarModelFactoryImpl
import com.cynosure.operit.data.repository.AvatarRepository
...
import com.cynosure.operit.util.streamnative.NativeXmlSplitter
...
import com.cynosure.operit.core.tools.ComputerDesktopActionResultData
...
import com.cynosure.operit.api.chat.llmprovider.MediaLinkBuilder
import com.cynosure.operit.data.repository.getCustomMoodDefinitions
import com.cynosure.operit.data.repository.getMoodAnimationMapping
        ...
        try {
            ...
            for (attempt in 1..maxRetries) {
                ...
                try {
                    ...
                    summaryContent = ChatUtils.removeThinkingContent(contentBuilder.toString().trim())
                } catch (e: Exception) {
                    AppLogger.w(TAG, "总结生成第 ${attempt} 次失败: ${e.message}", e)
                    if (attempt < maxRetries) {
                        kotlinx.coroutines.delay(1000L * attempt)
                    }
                }

                if (summaryContent.isNotBlank()) break

                if (attempt < maxRetries) {
                    ToolProgressBus.update(
                        ToolProgressBus.SUMMARY_PROGRESS_TOOL_NAME,
                        0.3f * attempt,
                        context.getString(R.string.conversation_summary_retrying, attempt, maxRetries)
                    )
                }
            }

            val progressMessage = if (summaryContent.isBlank()) {
                context.getString(R.string.conversation_summary_failed)
            } else {
            ...
            // 如果内容为空，返回空字符串让上层容错生效（而非占位串）
            if (summaryContent.isBlank()) {
                AppLogger.w(TAG, "总结生成 ${maxRetries} 次后仍为空，返回空字符串触发上层容错")
                return ""
            }
            ...
            AppLogger.e(TAG, "生成总结时出错（已重试 ${3} 次仍失败）", e)
            return ""
        }
    }
    ...
    suspend fun prepareConversationHistory(
        ...
        conversationMutex.withLock {
            ...
            if (!effectiveChatHistory.any { it.kind == PromptTurnKind.SYSTEM }) {
                ...
                val enableAntiHallucination = apiPreferences.enableAntiHallucinationFlow.first()
                val enableFactGuard = apiPreferences.enableFactGuardFlow.first()
                val enableOutputDiscipline = apiPreferences.enableOutputDisciplineFlow.first()
                ...
                val finalSystemPrompt = buildString {
                    if (enableAntiHallucination && enableFactGuard) {
                        append(AntiHallucinationGuard.HALLUCINATION_SYSTEM_PROMPT)
                        append("\n\n")
                    }
                    if (enableAntiHallucination && enableOutputDiscipline) {
                        append(AntiHallucinationGuard.OUTPUT_DISCIPLINE_SYSTEM_PROMPT)
                        append("\n\n")
                    }
    ...
    private suspend fun buildWaifuRulesText(): String {
        ...
        if (waifuEnableEmoticons) {
            ...
            val availableCategories = try {
                ...
                com.cynosure.operit.util.AppLogger.e("ConversationService", "获取表情分组失败", e)
                emptyList()
            }
...
${FunctionalPrompts.translationUserPrompt(targetLanguage, text)}
    ...
    suspend fun analyzeImageWithIntent(
        ...
        return try {
            ...
            // 添加图片到池子并获取ID
            val imageId = com.cynosure.operit.util.ImagePoolManager.addImage(imagePath)
            if (imageId == "error") {
            ...
            // 清理图片缓存
            com.cynosure.operit.util.ImagePoolManager.removeImage(imageId)
    ...
    suspend fun analyzeAudioWithIntent(
        ...
        return try {
            ...
            val mediaId = com.cynosure.operit.util.MediaPoolManager.addMedia(audioPath, mimeType)
            if (mediaId == "error") {
            ...
            com.cynosure.operit.util.MediaPoolManager.removeMedia(mediaId)
            ChatUtils.removeThinkingContent(result.toString()).trim()
        } catch (e: Exception) {
    ...
    suspend fun analyzeVideoWithIntent(
        ...
        return try {
            ...
            val mediaId = com.cynosure.operit.util.MediaPoolManager.addMedia(videoPath, mimeType)
            if (mediaId == "error") {
