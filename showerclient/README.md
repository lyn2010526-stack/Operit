...
广播协议（与主项目保持一致）：
 ...
 - **Action**：`com.cynosure.operit.action.SHOWER_BINDER_READY`
 - **Extra key**：`binder_container`
 - **Extra 类型**：`com.cynosure.shower.ShowerBinderContainer`（`Parcelable`，内部包含 `IBinder`）
...
class ShowerBinderReceiver : BroadcastReceiver() {
    ...
    companion object {
        const val ACTION_SHOWER_BINDER_READY =
            "com.cynosure.operit.action.SHOWER_BINDER_READY"
...
<receiver
    ...
    <intent-filter>
        <action android:name="com.cynosure.operit.action.SHOWER_BINDER_READY" />
    </intent-filter>
</receiver>
```
...
说明：
 ...
 - `ShowerServerManager` 启动命令会附带宿主包名参数：`CLASSPATH=/data/local/tmp/shower-server.jar app_process / com.cynosure.shower.Main <hostPackage> &`。
...
- `com.cynosure.showerclient.ShowerVideoRenderer`
- `com.cynosure.showerclient.ui.ShowerSurfaceView`
...
) : com.cynosure.showerclient.ui.ShowerSurfaceView(context, attrs)
```
